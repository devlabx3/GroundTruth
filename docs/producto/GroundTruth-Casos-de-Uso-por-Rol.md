...
- ⚠️ **O4 — Topología** (`topologia.gestionar`): alta de parcela (polígono en Leaflet, cultivo, nodos). **Gate de sensores impuesto por el SERVIDOR** con el área calculada por PostGIS — no por el navegador. ⚠️ **El umbral por defecto es 20.000 m² (2 ha por sensor), no 1/5.000 m²**; es configurable por el Admin. Rechaza polígonos inválidos (uno que se cruza a sí mismo tiene área, pero no es una parcela). **Los nodos nacen con la parcela** (un nodo "libre" no existe en el modelo). 🔜 Falta el **borrado/edición** de parcelas.
- ⚠️ **O5 — Agricultores** (`agricultores.gestionar`): crear un agricultor **crea su usuario y su finca** en la unidad. ⚠️ **No existe "vincular/desvincular agricultor↔finca"** sobre fincas ya creadas. 🔴 **Y el agricultor creado NO puede iniciar sesión todavía** (falta el flujo de invitación de Supabase Auth).
