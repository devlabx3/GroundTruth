...
/**
 * O bien un privilegio, o bien "cualquiera de estos". Ambos opcionales pero uno
 * es obligatorio: sin ninguno, la puerta quedaría abierta — por eso `allowed`
 * cae a `false` en vez de a `true` cuando no se pasa nada.
 */
export interface RequirePrivilegeProps {
  privilege?: Privilege;
  anyOf?: Privilege[];
  children: ReactNode;
}

export function RequirePrivilege({ privilege, anyOf, children }: RequirePrivilegeProps) {
  const can = useSession((s) => s.can);
  const { t } = useTranslation('errors');
  const allowed = anyOf ? anyOf.some((p) => can(p)) : privilege ? can(privilege) : false;
  if (!allowed) {
    return (
