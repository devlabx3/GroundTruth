...
import type { SagaStep, StepStatus } from '@/types/ui';

export interface OnchainProgressModalProps {
  open: boolean;
  titleKey: string;
  steps: SagaStep[];
  /** Regla 1: solo `true` cuando ningún paso está activo. */
  canDismiss?: boolean;
  onDismiss?: () => void;
  certified?: boolean;
}
              ...
              <div className="flex-1">
                ...
                {step.status === 'failed' && (
                  <div className="mt-1 text-xs text-sealwax">
                    {t(step.errorKey ?? 'server', { ns: 'errors' })}
                  </div>
                )}
  ...
  if (status === 'done')
    return (
      <span className="grid h-[22px] w-[22px] shrink-0 place-items-center rounded-full bg-emerald">
        <CheckCircleIcon size={16} weight="fill" color="#F7F5F0" />
      </span>
    );
  if (status === 'active')
